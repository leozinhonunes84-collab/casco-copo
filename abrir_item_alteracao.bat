...
set "PORT=4177"
set "HOST=0.0.0.0"
set "URL=http://127.0.0.1:4177/item-alteracao.html"

echo Verificando se o painel ja esta em execucao...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r=Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:4177/item-alteracao.html' -TimeoutSec 1; if ($r.StatusCode -eq 200 -and $r.Content -notmatch 'Rota nao encontrada') { exit 0 } else { exit 1 } } catch { exit 1 }"
...
start "ZigPay Item Alteracao - Servidor" "%PYTHON_CMD%" -u app.py

echo Aguardando o painel responder...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$url='http://127.0.0.1:4177/item-alteracao.html'; $end=(Get-Date).AddSeconds(20); do { try { $r=Invoke-WebRequest -UseBasicParsing -Uri $url -TimeoutSec 1; if ($r.StatusCode -eq 200 -and $r.Content -notmatch 'Rota nao encontrada') { exit 0 } } catch { Start-Sleep -Milliseconds 400 } } while ((Get-Date) -lt $end); exit 1"

if errorlevel 1 (
  echo.
  echo O servidor nao respondeu na porta %PORT%.
  echo Veja a janela "ZigPay Item Alteracao - Servidor" para o erro.
  echo.
  pause
  exit /b 1
)
